...
use cucumber::{given, then, when, gherkin::Step, StatsWriter, World};
...
impl GameWorld {
    ...
    async fn mcp_call(&self, tool: &str, args: Value) -> Result<Value, String> {
        ...
        let resp: Value = self.http_client
            .post(format!("{}/mcp", self.base_url))
            .json(&payload)
            .send().await.map_err(|e| format!("MCP 请求失败: {}", e))?
    ...
    async fn ensure_server(&mut self) {
        ...
        while let Some(wait) = backoff::backoff::Backoff::next_backoff(&mut backoff) {
            if self.http_client.get(format!("{}/health", self.base_url))
    ...
    async fn get_node(&self, character_id: &str, schema_id: &str) -> Result<Value, String> {
        self.mcp_call("get_node_value", json!({
    ...
    async fn get_edges(&self, character_id: &str) -> Result<Vec<Value>, String> {
        let result = self.mcp_call("get_edges", json!({ "character_id": character_id })).await?;
        result.as_array().cloned().ok_or("边结果不是数组".to_string())
    }
}
...
async fn advance_ticks(world: &mut GameWorld, count: u64) {
    let result = world.mcp_call("tick", json!({ "count": count, "dt": 1.0 })).await
        .expect("推进 tick 失败");
    world.last_result = Some(result);
}
...
async fn execute_command(world: &mut GameWorld, cmd: String, actor: String, target: String) {
    let result = world.mcp_call("execute_command", json!({
...
async fn repeat_training_with_table(world: &mut GameWorld, step: &Step, target: String, rounds: u64) {
    ...
    for _ in 0..rounds {
        for row in &table.rows[1..] { // skip header
            ...
            // 先执行命令
            world.mcp_call("execute_command", json!({
                "command_id": cmd, "actor_id": actor, "target_id": &target,
            })).await.expect(&format!("执行命令 {} 失败", cmd));
            // 再推进 tick
            world.mcp_call("tick", json!({ "count": ticks, "dt": 1.0 })).await
                .expect("推进 tick 失败");
        }
    }
}
...
async fn repeat_single_command(
    world: &mut GameWorld, repeat: u64, cmd: String, actor: String, target: String, ticks: u64,
) {
    for _ in 0..repeat {
        world.mcp_call("execute_command", json!({
            "command_id": &cmd, "actor_id": &actor, "target_id": &target,
...
async fn advance_ticks_no_action(world: &mut GameWorld, count: u64) {
    world.mcp_call("tick", json!({ "count": count, "dt": 1.0 })).await
        .expect("推进 tick 失败");
}
...
async fn node_value_gt(world: &mut GameWorld, character: String, schema: String, threshold: f64) {
    let node = world.get_node(&character, &schema).await.expect("查询节点失败");
    let value = node["value"].as_f64().expect(&format!("节点 {} 无 value: {:?}", schema, node));
    assert!(value > threshold, "{} 的 {} 值 {:.3} 应 > {}", character, schema, value, threshold);
}
...
async fn node_exists(world: &mut GameWorld, character: String, schema: String) {
    let node = world.get_node(&character, &schema).await.expect("查询节点失败");
    assert!(node.get("found") != Some(&json!(false)),
        "{} 的节点 {} 应该存在: {:?}", character, schema, node);
}

#[then(expr = "{string} 的 {string} 节点应激活")]
async fn node_active(world: &mut GameWorld, character: String, schema: String) {
    let node = world.get_node(&character, &schema).await.expect("查询节点失败");
    assert_eq!(node["active"], true, "{} 的 {} 应激活: {:?}", character, schema, node);
}

#[then(expr = "{string} 的 {string} 节点应未激活")]
async fn node_inactive(world: &mut GameWorld, character: String, schema: String) {
    let node = world.get_node(&character, &schema).await.expect("查询节点失败");
    assert_eq!(node["active"], false, "{} 的 {} 应未激活: {:?}", character, schema, node);
}

/// 检查两个节点之间是否存在边，且权重满足条件
#[then(expr = "{string} 中从 {string} 到 {string} 的边权重应大于 {float}")]
async fn edge_weight_gt(
    world: &mut GameWorld, character: String, source: String, target: String, threshold: f64,
) {
    let edges = world.get_edges(&character).await.expect("查询边失败");
    let edge = edges.iter().find(|e|
        e["source_instance_id"].as_str().map_or(false, |s| s.contains(&source)) &&
        e["target_instance_id"].as_str().map_or(false, |t| t.contains(&target))
    );
    assert!(edge.is_some(), "{} 中未找到从 {} 到 {} 的边", character, source, target);
    let weight = edge.unwrap()["weight"].as_f64().unwrap_or(0.0);
    assert!(weight > threshold, "{} 中 {} → {} 边权重 {:.3} 应 > {}", character, source, target, weight, threshold);
}
...
async fn edge_weight_lt(
    ...
    // 如果边不存在，权重视为 0
    let weight = edge.map(|e| e["weight"].as_f64().unwrap_or(0.0)).unwrap_or(0.0);
    assert!(weight < threshold, "{} 中 {} → {} 边权重 {:.3} 应 < {}", character, source, target, weight, threshold);
}
...
async fn has_learnable_edges(world: &mut GameWorld, character: String, min_count: usize) {
    ...
    assert!(learned >= min_count, "{} 应至少有 {} 条可学习边，实际 {}", character, min_count, learned);
}
...
async fn has_learn_type_edge(world: &mut GameWorld, character: String, learn_type: String) {
    let edges = world.get_edges(&character).await.expect("查询边失败");
    let found = edges.iter().any(|e| e["learn_type"].as_str() == Some(learn_type.as_str()));
    assert!(found, "{} 应有 {} 类型的学习边", character, learn_type);
}

/// 检查可用命令是否包含指定命令
#[then(expr = "执行者 {string} 对目标 {string} 的可用命令应包含 {string}")]
async fn available_cmd_contains(world: &mut GameWorld, actor: String, target: String, cmd_id: String) {
    let cmds = world.mcp_call("list_commands", json!({
    ...
    assert!(found, "可用命令应包含 {}，但只有: {:?}", cmd_id,
        cmds_array.iter().map(|c| c["command_id"].as_str().unwrap_or("?")).collect::<Vec<_>>());
}

/// 检查可用命令中不应包含指定命令（前置条件未满足）
#[then(expr = "执行者 {string} 对目标 {string} 的可用命令不应包含 {string}")]
async fn available_cmd_not_contains(world: &mut GameWorld, actor: String, target: String, cmd_id: String) {
    let cmds = world.mcp_call("list_commands", json!({
        "actor_id": actor, "target_id": target,
    })).await.expect("查询命令失败");
    let cmds_array = cmds.as_array().expect("命令结果不是数组");
...
async fn world_has_entities(world: &mut GameWorld, step: &Step) {
    let table = step.table.as_ref().expect("此步骤需要 DataTable");
    let chars = world.mcp_call("get_characters", json!({})).await.expect("查询角色失败");
    let items = world.mcp_call("get_items", json!({})).await.expect("查询物品失败");
    ...
    for row in &table.rows[1..] { // skip header
        ...
        match entity_type.as_str() {
            "角色" => {
                assert!(chars_arr.iter().any(|c| c["id"] == *entity_id),
                    "角色 {} 应存在，但角色列表: {:?}", entity_id,
                    chars_arr.iter().map(|c| c["id"].as_str().unwrap_or("?")).collect::<Vec<_>>());
            }
            "物品" => {
                assert!(items_arr.iter().any(|i| i["id"] == *entity_id),
                    "物品 {} 应存在，但物品列表: {:?}", entity_id,
                    items_arr.iter().map(|i| i["id"].as_str().unwrap_or("?")).collect::<Vec<_>>());
            }
...
async fn check_nodes_table(world: &mut GameWorld, step: &Step, character: String) {
    let table = step.table.as_ref().expect("此步骤需要 DataTable");
    for row in &table.rows[1..] { // skip header: | schema_id | 条件 | 阈值 |
        let schema = &row[0];
        let condition = &row[1];
        let threshold: f64 = row[2].parse().expect(&format!("阈值 '{}' 不是数字", row[2]));
        let node = world.get_node(&character, schema).await
