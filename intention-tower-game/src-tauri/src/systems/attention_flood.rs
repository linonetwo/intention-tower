...
impl System for AttentionFloodSystem {
    fn name(&self) -> &'static str { "AttentionFloodSystem" }
    ...
    fn run(&self, state: &mut WorldState, _dt: f64) {
        for character in state.characters.values_mut() {
            ...
            let floods: Vec<(String, f64)> = character.mind_graph.nodes.values()
                .filter(|n| {
            ...
            for (_flood_id, drain_rate) in floods {
                ...
                let old = character.mind_graph.resource_value(ATTENTION_SCHEMA);
                character.mind_graph.consume_resource(ATTENTION_SCHEMA, drain_rate);
